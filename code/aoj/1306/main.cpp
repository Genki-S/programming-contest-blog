...
int main(int argc, char** argv) {
	...
	while (cin >> n, n) {
		...
		ULL ans = INF;
		VVULL dp(n, VULL(4, INF));
		if (balloons[0].F > balloons[0].S) {
			cout << "NG 1" << endl;
			goto NG;
		}
		dp[0][1] = balloons[0].F;
		dump(balloons[0]);
		dprt("Get first ballon with %llu moves\n", dp[0][1]);
		FOR (i, 1, n) {
			int ppos = balloons[i-1].F;
			int ptime = balloons[i-1].S;
			int cpos = balloons[i].F;
			int ctime = balloons[i].S;
			dump(balloons[i]);
			FOR (j, 1, 3) {
				if (dp[i-1][j] == INF) { continue; }
				if ((j+1) * abs(cpos - ppos) <= ctime - ptime) {
					dp[i][j+1] = min(dp[i][j+1], dp[i-1][j] + abs(cpos - ppos));
					dprt("Get %d-th balloon with %llu moves with %d carrying balloons\n", i+1, dp[i][j+1], j+1);
				}
			}
			FOR (j, 1, 4) {
				// player can unload balloons
				if (dp[i-1][j] == INF) { continue; }
				if ((j+1) * ppos + cpos <= ctime - ptime) {
					dp[i][1] = min(dp[i][1], dp[i-1][j] + ppos + cpos);
					dprt("Get %d-th balloon with %llu moves with %d carrying balloons\n", i+1, dp[i][1], 1);
				}
			}
		...
		dumpl(dp);
		FOR (j, 1, 4) {
			ans = min(ans, dp[n-1][j] + balloons[n-1].F);
		}
		cout << "OK " << ans << endl;
NG:
		;
