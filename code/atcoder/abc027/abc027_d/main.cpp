...
// unordered_map
// template<typename T1, typename T2> ostream& operator<<(ostream& s, const unordered_map<T1, T2>& m) {
	// s << "{" << endl;
	// for (typeof(m.begin()) itr = m.begin(); itr != m.end(); ++itr) {
		// s << "\t" << (*itr).first << " : " << (*itr).second << endl;
	// }
...
int main(int argc, char** argv) {
	...
	REP (i, len) {
		...
		EACH (dp, itr) {
			int pos = (*itr).first;
			if (abs(pos) > moves + 1) { continue; }

			if (c == 'M') {
				if (!EXIST(ndp, pos+1)) { ndp[pos+1] = -INF; }
				if (!EXIST(ndp, pos-1)) { ndp[pos-1] = -INF; }
				ndp[pos+1] = max(ndp[pos+1], dp[pos]);
				ndp[pos-1] = max(ndp[pos-1], dp[pos]);
			} else if (c == '+') {
				ndp[pos] = dp[pos] + pos;
			} else if (c == '-') {
